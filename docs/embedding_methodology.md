...
## Vector search in Postgres (tasks 112-115)

`qb_reference_features.embedding_vector` is a real `pgvector` column (`vector(8)`, matching `EMBEDDING_DIM`), not a JSON placeholder -- with an HNSW index (`vector_cosine_ops`, matching `pipeline/similarity_v2.py::embedding_similarity()`'s cosine metric). `db/vector_search.py::find_nearest_by_embedding()` queries that index directly via pgvector's SQLAlchemy comparator instead of pulling every reference row into Python and scoring it in a loop -- the latter is still what the V0/V1 `compare_features` layer does (it operates on the JSON `feature_vector`, which isn't a DB-indexable type), but the V2 embedding layer doesn't need to. `db/backfill_embeddings.py` fills in `embedding_vector` for existing rows once a trained checkpoint exists for their phase, skipping phases without one rather than erroring.

Verified against the real local Postgres (task 63): the extension installs and enables, the migration's drop-and-recreate correctly produces a real `vector(8)` column (autogenerate's ALTER-TYPE approach doesn't work here -- there's no built-in Postgres cast from JSON to vector), the HNSW index actually gets used by a real nearest-neighbor query (confirmed it returns the correct cluster on synthetic embeddings), and the backfill script correctly populates rows using a real trained-and-exported checkpoint while leaving phases with no checkpoint untouched. Both migrations verified in both directions (upgrade/downgrade/re-upgrade).
